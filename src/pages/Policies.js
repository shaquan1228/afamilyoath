      ...
      <div className="business-logic-right">
        {
          this.state.info.map((item, i) => {
            return (
              (i % 2 === 1) 
              ? <InfoSquare title={item.title} img={item.imgUrl} innerHTML={item.text} style="img_left" />
              : <InfoSquare title={item.title} img={item.imgUrl} innerHTML={item.text} style="img_right" />
            )
          })
        }
      </div>
